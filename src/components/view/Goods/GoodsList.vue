<template>
 ...
 <!-- <el-alert
    ...
    type="warning"
    show-icon :closable="false" v-if="logingoods.isAdmin<2">
  </el-alert> -->
    <el-row>
...
<el-dialog title="修改商品" :visible.sync="editgoodsVisible">
  <el-form :model="form" ref="loginFormRef" :rules="rules">
    <el-form-item  label="商品名" prop="tag" :label-width="formLabelWidth">
      <el-input v-model="nowgoods.account"  autocomplete="off"></el-input>
    </el-form-item>
    <el-form-item  label="价格" prop="price" :label-width="formLabelWidth">
      <el-input v-model="nowgoods.uname"  autocomplete="off"></el-input>
    </el-form-item>
    <el-form-item  label="描述" prop="desc" :label-width="formLabelWidth">
      <el-input v-model="nowgoods.passwd"  autocomplete="off"></el-input>
    </el-form-item>
...
export default {
  ...
  methods: {
    tableRowClassName({row, rowIndex}) {
        let logingoods = JSON.parse(localStorage.getItem('logingoods'))
        if (row.admin_tag === logingoods.tag) {
            return 'warning-row'
        }
